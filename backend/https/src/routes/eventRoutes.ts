...
eventRoutes.get("/my-events", authMiddleware, async (req: Request & { user?: any }, res: Response) => {
	try {
		...
		const processedCreatedEvents = createdEvents.map(event => ({
			...event,
			permission: 'Owner',
			isCreator: true
		}));
		...
		const processedAssignedEvents = assignedEvents.map(event => {
			// Find user's assignment for this event
			const userAssignment = event.assignments.find(a => a.userId === userId);
			...
			if (userAssignment.level === 'Member') {
				const assignedCategoryIds = userAssignment.categories.map(c => c.id);
				return {
					...event,
					categories: event.categories.filter(c => assignedCategoryIds.includes(c.id)),
					permission: 'Member',
					isCreator: false
				};
			}
		...
		processedAssignedEvents.forEach(assignedEvent => {
			if (!allEvents.some(event => event.eventId === assignedEvent.eventId)) {
...
// Delete event
eventRoutes.delete("/:eventId", authMiddleware, (async (req: Request, res: Response) => {
	const { eventId } = req.params;
	try {
...
eventRoutes.post("/:eventId/confirm-delivery", authMiddleware, (async (req: Request & { user?: { id: string } }, res: Response) => {
	...
	try {
		...
		const allItemsDelivered = event.categories.every(category =>
			category.items.every(item => item.isDelivered)
		);
		...
		const allItemsPacked = event.categories.every(category =>
